...
const UserMessage = () => {
  ...
  const addRecipient = () => {
    ...
    if (!/^\d+-\d+@g\.us$/.test(currentNumber.trim())) {
      alert(
        "Format ID Grup tidak valid. Contoh: 6289687900722-1506428022@g.us",
      );
      return;
    }
    ...
    if (recipients.includes(currentNumber.trim())) {
      alert("Grup sudah ada dalam daftar");
      return;
    }
  ...
  const clearAllRecipients = () => {
    if (confirm("Hapus semua Grup penerima?")) {
      setRecipients([]);
    }
  };
  ...
  const submitHandler = async (e) => {
    ...
    if (recipients.length === 0) {
      alert("Tambahkan minimal 1 Grup penerima");
      return;
    }
    ...
    for (let i = 0; i < recipients.length; i++) {
      ...
      try {
        await api.post("/whatsapp/send/group", {
          deviceId,
          groupId: phoneNumber,
          message,
        });
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              Kirim pesan WhatsApp ke banyak Grup sekaligus
            </p>
          </div>
          {recipients.length > 0 && (
                ...
                {recipients.length === 0 ? (
                  <div className="text-center py-3 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <p className="text-gray-400 font-medium text-sm">
                      Belum ada Grup penerima
                    </p>
                    <p className="text-gray-400 text-xs mt-1">
                      Tambahkan Grup untuk memulai
                    </p>
                  </div>
                ) : (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-emerald-600">
                    {recipients.length} Grup WhatsApp
                  </p>
                </div>
              </div>
            </div>
